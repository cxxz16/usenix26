...
class Ast2CodeFactory(object):
    @staticmethod
    def extract_code(analyzer: Neo4jEngine, feeder: Union[List, int, py2neo.Node], with_memory=True, normalize_level=0, arg_count=1):
        ...
        if isinstance(feeder, List) and isinstance(feeder[0], int):
            ...
            if normalize_level >= 2 and res:
                for i in range(1, arg_count+1):
                    res[-i] = re.sub(r"(\$[a-zA-Z0-9_]+)\s*(->[a-zA-Z0-9_]+)([^a-zA-Z0-9_])", 
                                Ast2CodeFactory.repl_chained_var, res[-i])
                    res[-i] = re.sub(r"(\$[a-zA-Z0-9_]+)\s*(\[['\"].*?['\"]\])?", 
                                Ast2CodeFactory.repl_var, res[-i])
            return res
