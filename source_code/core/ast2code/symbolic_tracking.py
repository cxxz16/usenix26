...
class SymbolicTracking(object):
    ...
    def var_in_tac_mem(self, var: str):
        for idx, _mem in enumerate(self.tac_path.get_mem()):
            assert isinstance(_mem, ThreeAddressCode)
            if _mem.formula.left == var:
                return idx - self.tac_path.__len__() 
        return -1
    ...
    def manage_function_call(self, node: py2neo.Node):
        args = self.analyzer.ast_step.find_function_arg_node_list_old(node)
        resolved_args = []
        resolved_args_types = []
        for arg in args:
            assert isinstance(arg, py2neo.Node)
    ...
    def manage_binary_operation(self, node: py2neo.Node):
        ...
        elif (node['flags'][0] in [BINARY_COALESCE]):
            # $_GET['page'] ?? 1;
            return self.gen_formula(join_var_node(node.identity), [l_expr[0], r_expr[0]], node['flags'][0], 
                                    node[NODE_TYPE],
                                    node.identity, "boolean",
    ...
    def manage_generic_node(self, node: py2neo.Node):
        if node[NODE_TYPE] in {TYPE_CALL, TYPE_DIM, TYPE_VAR, TYPE_METHOD_CALL, TYPE_STATIC_CALL, TYPE_PROP, TYPE_ARRAY, "string", "integer", "double", TYPE_CONST, TYPE_MAGIC_CONST, TYPE_STATIC_PROP,
