...
class CVESinkFinder(object):
    ...
    def traversal(self, level=0) -> bool:
        ...
        def traversal_recur(level):
            if level > self.max_caller_depth:       # 这个 level 决定向 caller 方向找的深度
                return False
            # if self.load_sinks():
            #     return True
            # 前向遍历找到 sink，这个函数的主要目标就是找 sink  这个过程在找 callee 方向的 sink
            ...
            if self.potential_anchor_nodes.__len__() == 0:
                ...
                if self.__level_delay_node[str(level)].__len__() >= 1 \
                    ...
                    for i in self.__level_delay_node[str(level)]:
                        ...
                        func_node = self.analyzer.basic_step.get_node_itself(i[NODE_FUNCID])    # 获取当前函数节点
                        if func_node == None: continue
                        # 当 caller 进行分析的时候，跳过当前patch 所在函数
                        if level == 0:
                        ...
                        if func_node[NODE_TYPE] in {TYPE_FUNC_DECL, TYPE_METHOD}:
                            call_sites = self.analyzer.cg_step.find_call_nodes(func_node)   # 找到所有 call 当前函数的
                            if 1 <= call_sites.__len__() and call_sites.__len__() <= 10:
                                call_sites = sorted(call_sites, key=lambda x: x.identity)[:3]  # 只取前3个 callsite 进行分析，防止爆炸
                ...
                else:   # 如果不存在 return  不需要往caller 方向
                    if level > 0:
                        self._record_current_caller_path()
                        self.call_paths.clear()
                        self._pop_caller_stack()
