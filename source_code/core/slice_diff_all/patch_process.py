...
def find_nearest_function_to_changes(lines, changed_line_indices):
    ...
    Args:
        lines: diff的所有行
        changed_line_indices: 有变更（+/-）的行的索引列表
            ...
            if match:
                ...
                if func_name and func_name not in c_keywords:
                    ...
                    if func_name in blacklist:
                        continue
                    # print(f"DEBUG: 找到函数 '{func_name}' 在行 {i}: {clean_line.strip()}")
                    return func_name
...
def extract_function_from_hunk_header(hunk_header):
    """
    从hunk header中提取函数名（作为备选方案）
    示例: @@ -2766,7 +2766,7 @@ mrb_vm_exec(mrb_state *mrb, const struct RProc *proc, const mrb_code *pc)
    """
    # 匹配 @@ ... @@ 后面的内容
    pattern = r'@@.*?@@\s*(.+)$'
    ...
    if match:
        ...
        # 尝试提取函数名
        # 匹配 function_name(params) 格式
        func_match = re.search(r'([a-zA-Z_][a-zA-Z0-9_]*)\s*\(', context)
        if func_match:
            func_name = func_match.group(1)
            # print(f"DEBUG: 从hunk header提取函数名: {func_name}")
            return func_name
...
def parse_diff(diff_content):
    ...
    current_hunk_changes = []  # 当前hunk中的变更行索引
    ...
    while i < len(lines):
        ...
        if hunk_match:
            ...
            if current_hunk_start is not None and current_hunk_changes:
                # print(f"DEBUG: 处理hunk，变更行索引: {current_hunk_changes}")
    ...
    # 处理最后一个hunk
    if current_hunk_start is not None and current_hunk_changes:
        # print(f"DEBUG: 处理最后一个hunk，变更行索引: {current_hunk_changes}")
        func_name = find_nearest_function_to_changes(lines, current_hunk_changes)
        if func_name and func_name not in result["functions"]:
            result["functions"].append(func_name)
    ...
    if not result["functions"] and changed_line_indices:
        # print(f"DEBUG: 从整体变更中寻找函数，变更行索引: {changed_line_indices}")
...
"""
    ...
    print("\n=== 结果 ===")
    print("提取的函数:", result["functions"])
    print("从header提取的函数:", result["header_functions"])
    print("添加的行数:", result["added"])
    print("删除的行数:", result["deleted"])
    print("-" * 150)
