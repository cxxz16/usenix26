...
def create_dir_if_exists(PATH: str | list):
...
def get_expression_and_conditions(analyzer: Neo4jEngine, path: List[int], condition_ids: List[int]):
        ...
        arg_list = analyzer.ast_step.find_function_arg_node_list_old(analyzer.get_node_itself(sink_node_id))
        for arg_node in arg_list:
            path_copy.append(arg_node[NODE_INDEX])
        arg_expressions = Ast2CodeFactory.extract_code(analyzer, path_copy, normalize_level=2, arg_count=len(arg_list))[-len(arg_list):]
