...
def convert_path_to_postpatch_name(path):
...
class PatchAnalyzer(object):
    ...
    def __init__(self, analysis_framework_pre: Neo4jEngine,
                 ...
                 commit_url, commit_id, prepatch_commit_id=None, cve_id=None, fixing_file=None):
        ...
        self.fixing_file = fixing_file
        self.source_code_helper = SourceCodeHelper()
        self.prepatch_commit_id = prepatch_commit_id if prepatch_commit_id is not None else PrePatchFinder.find_pre_commit(self.repository, self.commit_id)
        self.cve_id = cve_id
    ...
    def do_diff_local(self, file1, file2, diff_file):
        try:
    ...
    def find_modify_lines_from_diff(self):
        ...
        if not os.path.exists(diff_storage_path):
            self.do_diff(diff_storage_path)
            # self.do_diff_local(diff_storage_path)
        # return
        ...
        for k, v in r.items():
            ...
            if file_name is None:
                continue
            if file_name != self.fixing_file:
                continue
            start_line = []
            if file_name.split('.')[-1] not in PHP_EXTENSION:
                continue
            file_name = convert_path_to_postpatch_name(file_name)
            for _v in v:
    ...
    def _affected_line_transformer(self, lines, file_name, file_path):
        rt_result = set()
        file_id_pre = self.analyzer_pre.fig_step.get_file_name_node(file_name, match_strategy=3, file_path=file_path)[NODE_INDEX]
        for line in lines:
            # for _node in self.analyzer_pre.basic_step.match(LABEL_AST, **{
            #         NODE_FILEID: file_id_pre,
            #         NODE_LINENO: line,
            # }):
            for _node in self.analyzer_pre.basic_step.match(LABEL_AST, ).where(
                f"_.{NODE_FILEID}={file_id_pre} AND _.{NODE_LINENO}={line}"
            ):
    ...
    def _affected_line_transformer_ext(self, lines, file_name):
        file_id_post = self.analyzer_post.fig_step.get_file_name_node(file_name, match_strategy=2)[NODE_FILEID]
        file_id_pre = self.analyzer_pre.fig_step.get_file_name_node(file_name, match_strategy=2)[NODE_FILEID]
        rt_result = set()
        _result = set()
        for line in lines:
            ...
            #         NODE_CHILDNUM: 0
            # })
            _res = self.analyzer_post.basic_step.match(LABEL_AST, ).where(
                f"_.{NODE_FILEID}={file_id_post} AND _.{NODE_LINENO}={line} AND _.{NODE_CHILDNUM}=0"
            ).first()
        ...
        for root_node in _result:
            ...
            # elif root_node[NODE_TYPE] in {TYPE_UNARY_OP}:
            #     child_node = self.analyzer_post.ast_step.get_child_node(root_node)
            #     _result.append(child_node)
        return rt_result
    ...
    def __search_affected_param(self, _affect_param, file_name, lines_min, lines_max, file_id_pre):
        ...
        #                                                       file_name=file_name)
        source_path = self.analyzer_pre.fig_step.get_node_from_file_system(self.analyzer_pre.get_node_itself(file_id_pre))["name"]
    ...
    def run_result(self):
        res_dir = os.path.join(STORAGE_PATH, "patch_analysis_result", "results")
        res_file = os.path.join(res_dir, f"res_{self.commit_id}.json")
        if os.path.exists(res_file):
            print(f"[ 2.0 ] Patch analysis result exists: {res_file}")
            return
