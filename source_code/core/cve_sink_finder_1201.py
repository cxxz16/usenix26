...
class CVESinkFinder(object):
    ...
    def __init__(self, analysis_framework: Neo4jEngine, git_repository, vuln_type: int,
                 commit_id = None, config_level=None, cve_id=None, custom_sinks: List[str] = None, max_caller_depth = 1, max_callee_depth = 1, source_info=None, sink_info=None):
        ...
        self._f_insert = lambda n, func_name=None, judge_type=0b0001, loc=-1: self.potential_anchor_nodes.add(
                AnchorNode.from_node_instance(
                        ...
                        func_name=func_name if func_name is not None else self.analyzer.code_step.get_node_code(n), param_loc=loc,
    ...
    def _anchor_function_analysis(self, node: py2neo.Node, TAINT_DYNAMIC_CALL_FLAG: bool = None) -> int:
        ...
        if node[NODE_TYPE] in {TYPE_ECHO, TYPE_PRINT}:
            if self.anchor_functions == FUNCTION_MODEL[10]:
                nn = self.analyzer.ast_step.filter_child_nodes(_node=node, node_type_filter=VAR_TYPES_EXCLUDE_CONST_VAR | SET_FUNCTION_CALL, max_depth=10)
                if nn.__len__() >= 1:
                    return 0b10
                else:
                    return 0b00
    ...
    def _handle_call_node(self, node, taint_var, depth=0):
        ...
        if node[NODE_TYPE] in {TYPE_METHOD_CALL, TYPE_STATIC_CALL}:
            ...
            if method_decls:
                ...
                ana_result = self._anchor_function_analysis(method_name_node)
                if ana_result == 0b10:
                    self._f_insert(node, )
        ...
        elif node[NODE_TYPE] in {TYPE_CALL, TYPE_NEW}:
    ...
    def traversal(self, level=0) -> bool:
        def traversal_recur(level):
            ...
            sink_file_path = self.sink_info['file_path']
            sink_function_name = self.sink_info['function_name']
            sink_line_number = self.sink_info['line_number']

            filename = sink_file_path.split('/')[-1].split('.php')[0]
            sink_fs_node = self.analyzer.fig_step.get_file_name_node(filename, match_strategy=3, file_path=sink_file_path)

            query = f"MATCH (n: AST) WHERE n.fileid = {sink_fs_node[NODE_INDEX]} AND n.lineno = {sink_line_number} AND n.type in {COMMON_NODE_TYPES} RETURN n"
            print(query)
            sink_nodes = [n for n, in self.analyzer.basic_step.run(query)]

            if sink_nodes.__len__() == 1:
            ...
            for node in sink_nodes:
                call_node_name = None
                if node[NODE_TYPE] in {TYPE_METHOD_CALL, TYPE_STATIC_CALL}:
                    call_node_name = self.get_method_call_name(node)
                else:
                    call_node_name = self.analyzer.code_step.get_node_code(node)

                if call_node_name == sink_function_name:
                    self._f_insert(node, func_name=sink_function_name)
